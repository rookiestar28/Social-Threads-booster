...
1. リポジトリを AI ツール環境に入れる
2. `/setup` で過去投稿を取り込む
3. 投稿を書き終えたら `/analyze` で判定する
4. 公開前に `/predict` で 24 時間の見込みを見る
5. 公開後は `/review` で 24h / 72h のチェックポイントを回収する
6. Threads API がある場合は `scripts/update_snapshots.py` で `snapshots[]` を更新する

最小ワークフロー：

```text
/setup -> /analyze -> /predict -> 投稿 -> /review
```

API ありのワークフロー：

```text
/setup -> /analyze -> /predict -> 投稿 -> update_snapshots.py -> /review
```
...
初期化は一度だけ実行すれば十分です。以降のデータ更新は `/review` モジュールで継続的に蓄積されます。Threads API が使える場合は、`scripts/update_snapshots.py` で `snapshots[]` と `performance_windows` も補完できます。

---

## データ更新モード

AK-Threads-Booster には、現実的に使える 2 つの更新モードがあります。
...
- `/setup` で初期トラッカーを作成
- 公開後に `/review` で 24h / 72h / 7d の結果を回収
- その時点の結果を使って予測を補正

### スナップショットモード

Threads API がある場合はこの方法を使います。

- `scripts/fetch_threads.py` で初回インポート
- `scripts/update_snapshots.py` で定期的に metrics を更新
- tracker に `snapshots[]` を蓄積
- `performance_windows` に 24h / 72h / 7d の近似値を保持
...
投稿を書いた後、内容を貼り付けて decision-first 分析を実行します。
...
- **Algorithm Red Lines**：降格につながる要素を先に確認
- **Decision Summary**：上振れ要因と抑制要因を先に整理
- **Highest-Upside Comparisons**：過去の高パフォーマンス投稿と比較
- **Suppression Risks**：良い投稿でも伸びを止める要素を確認
- **Style / Psychology / Algorithm / AI-Tone**：その後に詳細分析

`style_guide.md` がまだない場合でも、`tracker` だけで退化動作します。
...
公開後に使用。実際のパフォーマンスデータを回収し、予測と比較し、システムデータを更新します。`snapshots[]` と `performance_windows` がある場合は、それも参照します。
...
- 予測との比較、乖離原因の分析
- tracker とスタイルガイドを更新
- `snapshots[]` と `performance_windows` を反映
- 最適な投稿タイミングを提案
...
3. /topics             -- トピック推薦を確認
4. /draft [トピック]   -- 下書きを生成
5. /analyze [投稿]     -- 下書きまたは自作投稿を decision-first で分析
6. （分析に基づいて自分で調整）
7. /predict [投稿]     -- 公開前にパフォーマンスを予測
8. （公開）
9. `scripts/update_snapshots.py` -- API がある場合は snapshot 更新
10. /review             -- 公開24時間後にデータ回収
11. ステップ3に戻る
```

サイクルを重ねるごとに、システムの分析と予測の精度が向上します。`/voice` は一度だけ実行すれば十分です（投稿が増えた後に再実行して更新することも可能）。`/draft` は Brand Voice ファイルを自動参照します。API がない場合は `scripts/update_snapshots.py` を飛ばして `/review` だけで十分です。
