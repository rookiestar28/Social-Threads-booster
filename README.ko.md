...
가장 빨리 시작하려면 이 순서면 충분합니다.

1. 리포지토리를 AI 도구 환경에 넣기
2. `/setup` 으로 과거 게시물 가져오기
3. 글을 쓴 뒤 `/analyze` 로 판정하기
4. 게시 전에 `/predict` 로 24시간 구간 보기
5. 게시 후 `/review` 로 24h / 72h 체크포인트 수집
6. Threads API가 있으면 `scripts/update_snapshots.py` 로 `snapshots[]` 갱신

최소 워크플로우:

```text
/setup -> /analyze -> /predict -> 게시 -> /review
```

API가 있는 워크플로우:

```text
/setup -> /analyze -> /predict -> 게시 -> update_snapshots.py -> /review
```
...
초기화는 한 번만 실행하면 됩니다. 이후 데이터 업데이트는 `/review` 모듈로 지속적으로 축적됩니다. Threads API를 사용할 수 있다면 `scripts/update_snapshots.py` 로 `snapshots[]` 와 `performance_windows` 도 채울 수 있습니다.

---

## 데이터 업데이트 모드

AK-Threads-Booster에는 실제로 쓸 수 있는 2가지 업데이트 모드가 있습니다.

### 체크포인트 모드

API가 없을 때 쓰는 방식입니다.

- `/setup` 으로 초기 tracker 생성
- 게시 후 `/review` 로 24h / 72h / 7d 결과 수집
- 그 시점 결과로 예측값 보정

### 스냅샷 모드

Threads API가 있으면 이 방식을 씁니다.

- `scripts/fetch_threads.py` 로 첫 수집
- `scripts/update_snapshots.py` 로 주기적 metrics 갱신
- tracker 에 `snapshots[]` 축적
- `performance_windows` 에 24h / 72h / 7d 근사값 유지
...
게시물 작성 후 내용을 붙여넣어 decision-first 분석을 실행합니다.
...
- **Algorithm Red Lines**: 강등으로 이어질 수 있는 요소를 먼저 확인
- **Decision Summary**: 상승 요인과 억제 요인을 먼저 정리
- **Highest-Upside Comparisons**: 과거 고성과 글과 비교
- **Suppression Risks**: 좋은 글이어도 확산을 막는 요소 확인
- **Style / Psychology / Algorithm / AI-Tone**: 그 다음 상세 분석

`style_guide.md` 가 아직 없어도 `tracker` 만으로 축약 동작합니다.
...
게시 후 실제 성과 데이터를 수집하고 예측과 비교하고 시스템 데이터를 업데이트합니다. `snapshots[]` 와 `performance_windows` 가 있으면 그것도 함께 참조합니다.
...
- `snapshots[]` 와 `performance_windows` 반영
- 최적 게시 시간 제안
...
3. /topics             -- 주제 추천 확인
4. /draft [주제]       -- 초안 생성
5. /analyze [게시물]   -- 초안 또는 직접 작성한 게시물 decision-first 분석
6. (분석을 바탕으로 직접 수정)
7. /predict [게시물]   -- 게시 전 성과 예측
8. (게시)
9. `scripts/update_snapshots.py` -- API가 있으면 snapshot 갱신
10. /review            -- 게시 24시간 후 데이터 수집
11. 3단계로 돌아가기
```

사이클을 반복할수록 시스템의 분석과 예측 정확도가 올라갑니다. `/voice`는 한 번만 실행하면 됩니다(게시물이 쌓인 후 다시 실행하여 업데이트 가능). `/draft`는 Brand Voice 파일을 자동으로 참조합니다. API가 없다면 `scripts/update_snapshots.py` 는 건너뛰고 `/review` 만 써도 됩니다.
