...
1. Thêm repo này vào AI tool của bạn.
2. Chạy `/setup` để import bài đăng lịch sử.
3. Sau khi viết xong, dùng `/analyze`; trước khi đăng có thể dùng `/predict`.
4. Sau khi đăng, dùng `/review` để lấy checkpoint 24h và 72h.
5. Nếu có Threads API token, chạy `scripts/update_snapshots.py` để cập nhật snapshot.

## Chế độ cập nhật dữ liệu

- **Checkpoint mode**: dành cho mọi người. `/review` lấy các giá trị 24h, 72h và 7d để cập nhật nền dự đoán.
- **Snapshot mode**: dùng Threads API. `scripts/update_snapshots.py` ghi thêm `snapshots[]` và cập nhật các `performance_windows` gần nhất.
